document.addEventListener("DOMContentLoaded", function() {
    document.getElementById("saveAsPdf").addEventListener("click", function(event) {
        ...
        const row = {
            ...
            HVLRNo: document.querySelector('input[name="hvrlNo"]').value,
            Location: document.querySelector('input[name="location"]').value,
            TypeOfTesting: document.querySelector('select[name="typeOfTesting"]').value,
            
            
            ControlPanelCondition: document.querySelector('select[name="controlpanel"]').value,
            UPMovement: document.querySelector('select[name="UPmovement"]').value,
            PowerSupply: document.querySelector('select[name="powersupply"]').value,
            DownMovement: document.querySelector('select[name="downmovement"]').value,
            MainTakeOffValve: document.querySelector('select[name="maintakeoffvalue"]').value,
            LeftMovement: document.querySelector('select[name="leftmovement"]').value,
            MonitorSwitch: document.querySelector('select[name="MonitorSwitch"]').value,
            RightMovement: document.querySelector('select[name="RightMovement"]').value,
            WaterMOVOpen: document.querySelector('select[name="WaterMOVopen"]').value,
            JetMovement: document.querySelector('select[name="JetMovement"]').value,
            WaterMOVClosed: document.querySelector('select[name="WaterMOVClosed"]').value,
            SprayMovement: document.querySelector('select[name="SprayMovement"]').value,
            

            BoosterPumpPowerSupply: document.querySelector('select[name="PowerSupply1"]').value,
            BoosterPumpCondition: document.querySelector('select[name="ConditionofPump"]').value,
            RiserLineDrainValve: document.querySelector('select[name="Riserline"]').value,
            PumpVibration: document.querySelector('select[name="Pumpviabration"]').value,
        
            Remarks: document.querySelector('textarea[name="remarks"]').value,
            OtherObservations: document.querySelector('textarea[name="Otherobservations"]').value,
        
            Instrument: document.querySelector('input[name="instrument"]').value,
            Electrical: document.querySelector('input[name="electrical"]').value
        };
 ...
 function addHeader() {
  ...
  pdf.setFontSize(12);
  pdf.text('Testing of HVLR Report', 95, subtitleY + 2, { align: 'center' });
...
pdf.text('Doc No: HPCL MR/FRM/F&S/08200-11', textX, textY + 20);
...
pdf.text(value1, xValueCenter , positionY - textVerticalOffset); // Add a small offset for value text
 ...
 if (label2) {
     const xLabel2Center = 101; // Adjust to place second label box directly after first value box
...
pdf.setFontSize(label2FontSize);
 ...
 return positionY + 12; // Move down for the next row and return updated positionY
  ...
  const data = [row]; // Wrap formData in an array for processing
  for (const row of data) 
  {
    let positionY = addHeader(); // Add header for the page and get the new position
                    positionY = 60; // Additional gap if needed
                    const y = 115;
                    const ControlPanelBoardFontSize = 12; // Set the desired font size for "Remarks"

// Set the font size and add "Remarks" text
pdf.setFontSize(ControlPanelBoardFontSize);
                    const ControlPanelBoardText = '1.Control Panel Board';
                    // Add the text outside the box, above it
                    const xText = 10;
                    pdf.text(ControlPanelBoardText, xText, y - 20); // Adjust the x and y offsets as needed
...
positionY = fieldPair('HVLR No:', row.HVLRNo ?? '', 'Location:', row.Location ?? '', positionY);
positionY = fieldPair('Type of Testing:', row.TypeOfTesting ?? '', '', '', positionY); // Empty string as label if no pair for 'Type of Testing'

positionY = 110; // Additional gap if needed

// Use the nullish coalescing operator (??) to provide fallback values
positionY = fieldPair('Control Panel Condition:', row.ControlPanelCondition ?? '', 'UP Movement:', row.UPMovement ?? '', positionY);
positionY = fieldPair('Power Supply:', row.PowerSupply ?? '', 'Down Movement:', row.DownMovement ?? '', positionY);
positionY = fieldPair('Main Take Off Valve:', row.MainTakeOffValve ?? '', 'Left Movement:', row.LeftMovement ?? '', positionY);
positionY = fieldPair('Monitor Switch:', row.MonitorSwitch ?? '', 'Right Movement:', row.RightMovement ?? '', positionY);
positionY = fieldPair('Water MOV Open:', row.WaterMOVOpen ?? '', 'Jet Movement:', row.JetMovement ?? '', positionY);
positionY = fieldPair('Water MOV Closed:', row.WaterMOVClosed ?? '', 'Spray Movement:', row.SprayMovement ?? '', positionY);
...
positionY = 120;
  ...
  const y = 200;
  const TestingBoosterPumpText = '2.Testing Booster Pump';
  const  TestingBoosterPumpFontSize = 12; // Set the desired font size for "Remarks"

// Set the font size and add "Remarks" text
pdf.setFontSize( TestingBoosterPumpFontSize);
  const xText = 10;
  pdf.text( TestingBoosterPumpText, xText, y - 20); // Adjust the x and y offsets as needed
  
  // Use the nullish coalescing operator (??) to provide fallback values
  positionY = 192;
  positionY = fieldPair('Power Supply:', row.BoosterPumpPowerSupply ?? '', 'Pump Condition:', row.BoosterPumpCondition ?? '', positionY);
positionY = fieldPair('Riser Line Drain Valve:', row.RiserLineDrainValve ?? '', 'Pump Vibration:', row.PumpVibration ?? '', positionY);

// Add the "Remarks" text
const remarksText = '3. Remarks';
const remarksX = 10; // Adjust x position as needed
const remarksY = 238; // Position below the Maintenance section
const remarksFontSize = 12; // Set the desired font size for "Remarks"

// Set the font size and add "Remarks" text
pdf.setFontSize(remarksFontSize);
pdf.text(remarksText, remarksX, remarksY - 20); // Adjust the y offset as needed

// Create the textarea for Remarks
const remarksTextareaX = 7;
const remarksTextareaY = 225;
...
positionY = fieldPair(
    'Instrument:', 
    document.querySelector('input[name="instrument"]').value || '', 
    'Electrical:', 
    document.querySelector('input[name="electrical"]').value || '', 
    positionY
);
